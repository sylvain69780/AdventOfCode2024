internal class Solution
{
    ...
    internal string Run()
    {
        ...
        var diamond = Enumerable.Range(-20, 41).SelectMany(x => Enumerable.Range(-20, 41).Select(y => (x, y,d: Math.Abs(x)+Math.Abs(y))))
            .Where(p => p.d <= 20).ToList();
        for (int i = 0; i < path.Count; i++)
        {
            //Console.WriteLine($"Index: {i}, Element: {path[i]}");
            foreach (var (dx, dy,d) in diamond)
            {
                var next = (x: path[i].x + dx, y: path[i].y + dy);
                if (Sample(next) == '#')
                    continue;
                if (graph.Contains(next))
                    {
                        if ( path.IndexOf(next) - i - d >= 100)
                        {
                            score++;
                        }
                    }
